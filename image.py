...
class Image:
    def __init__(self, x_pixels=0, y_pixels=0, num_channels=0, filename=''):
        ...
        self.input_path = 'C:\Gentleman\Review 001/'
        self.output_path = 'C:\Gentleman\Review 001/'
        if x_pixels and y_pixels and num_channels:
            ...
            raise ValueError('You need to input either a filename OR specify the dimensions of an image')
    
    def read_image(self, filename, gamma=2.2):
        '''
        read PNG RGB image, return 3D numpy array organized along X, X, channel
    ...
    def write_image(self, output_file_name, gamma=2.2):
        '''
        3D numpy array (X, Y, channel) of values between 0 and 1 -> write to png
        '''
        ...
        self.array.resize(y, x, 3)  # we mutated the method in the first 


if __name__ == '__main__':
    im = Image(filename='lake.png')
    im.write_image('meh.png')